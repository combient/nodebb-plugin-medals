...
Helpers.saveMedal = async (medalData) => {
	...
	medal.name = medalObject.name;
	medal.dscription = medalObject.dscription;
	medal.icon = medalObject.icon;
	medal.iconColor = medalObject.iconColor;
	medal.medalColor = medalObject.medalColor;

	if (!medal) {
		await db.sortedSetAdd(MEDALS_SET_KEY, medalObject.timestamp, medalObject.uuid);
	}

	await db.setObject(createMedalKey(medalObject.uuid), medal);
